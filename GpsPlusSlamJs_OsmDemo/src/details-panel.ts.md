...
- **Closing deselects rather than merely hiding.** A panel hidden while its cell was still selected would make re-clicking the same cell appear to do nothing. Pinned by an e2e that closes and re-clicks.
  - **BOTH selections, and for two years it was only the cell.** `main.ts`'s `onClose` dispatched `cellSelected(undefined)` alone, so closing a REGION's panel left `selectedRegion` set — and clicking that same region again dispatched an identical value, so the panel stayed shut. The reducer makes the two mutually exclusive when **selecting**, which is not the same as clearing. Fixed 2026-08-07 and now pinned for regions too.
  - **Why nothing caught it:** with a page per test, no test ever clicked a region twice. It surfaced only when the e2e file moved to one shared page and a region-selecting test broke the test after it. Cross-test state is usually a liability; here it was the only thing that could see this.
