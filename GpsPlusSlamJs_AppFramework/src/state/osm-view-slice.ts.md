...
- Actions
  ...
  - `renderFailed(message)` — phase `error`, **snapshot and selection untouched**.
...
store.dispatch(osmView.actions.renderFailed('WebGL context lost'));
...
- `osm-view-slice.property.test.ts` — the invariants over arbitrary action sequences: `renderFailed` never touches the snapshot, `fetchFailed` always clears it, every reachable state survives a JSON round-trip, and position/category change only through their own actions.
