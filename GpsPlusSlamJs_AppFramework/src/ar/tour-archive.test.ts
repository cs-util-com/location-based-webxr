...
import { TourManifestValidationError } from './tour-manifest';
...
describe('tourManifestEntryOf', () => {
  it('finds the root manifest, tolerates one wrapping folder, prefers the shallowest', () => {
...
describe('readTourManifestFromEntries', () => {
  it('returns null when the archive has no manifest (a recorder zip is normal)', async () => {
    await expect(
      readTourManifestFromEntries(['session.json'], () =>
  ...
  it('parses the manifest the writer named', async () => {
    ...
    await expect(
      readTourManifestFromEntries([TOUR_MANIFEST_ENTRY], (name) => {
  ...
  it('REJECTS a manifest that exists but is broken - never a silently empty tour', async () => {
    await expect(
      readTourManifestFromEntries(['tour.json'], () =>
