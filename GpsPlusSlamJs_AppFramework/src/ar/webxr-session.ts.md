...
- `activeSession` — the per-session `ArSessionHandle` (**Stages 0–2** of the [staged ArSession refactor](../../../../gps-plus-slam/GpsPlusSlamJs_Docs/docs/2026-07-18-2045-webxr-session-arsession-handle-refactor-plan.md)). Owns this session's clusters: the resolved crash-isolation options, the session-end pair (`onSessionEnd` callback + `endRequestedByApp` discriminator), the frame diagnostics (`lastFrameTime`, `latestCameraTexture`/width/height, camera-access log latches), the three capture clusters mirroring the `ArSessionCallbacks` group names (`imageCapture`: manager + JPEG blit + callback slots incl. the injected quality analyzer; `depth`: sampler + lazy 256×192 rgb blit + callbacks; `cameraFrame`: source + lazy aspect-preserving blit + captureSize + callback), and — since Stage 2 — the `tracking` cluster (store + phase subscription + host callbacks; `tracking.store` is the ONE field mutated mid-session, by `rebindTrackingStore`). Always non-null: `initAR()` creates a fresh handle parameterized with the validated isolation options and the callbacks struct; `resetWebXRState()` tears down the outgoing handle's phase subscription, disposes its GPU-backed blits, and REPLACES the handle wholesale — one line resets every field on it, so new handle state can never be forgotten in the teardown. Stage 3 migrates the last cluster (scene graph + session) into the handle.
