...
- `activeSession` — the per-session `ArSessionHandle` (**Stages 0–1** of the [staged ArSession refactor](../../../../gps-plus-slam/GpsPlusSlamJs_Docs/docs/2026-07-18-2045-webxr-session-arsession-handle-refactor-plan.md)). Owns this session's clusters: the resolved crash-isolation options, the session-end pair (`onSessionEnd` callback + `endRequestedByApp` discriminator), the frame diagnostics (`lastFrameTime`, `latestCameraTexture`/width/height, camera-access log latches), and — since Stage 1 — the three capture clusters mirroring the `ArSessionCallbacks` group names: `imageCapture` (manager + JPEG blit + callback slots incl. the injected quality analyzer), `depth` (sampler + lazy 256×192 rgb blit + callbacks), and `cameraFrame` (source + lazy aspect-preserving blit + captureSize + callback). Always non-null: `initAR()` creates a fresh handle parameterized with the validated isolation options and the callbacks struct; `resetWebXRState()` disposes the outgoing handle's GPU-backed blits and REPLACES the handle wholesale — one line resets every field on it, so new handle state can never be forgotten in the teardown. Later stages migrate the remaining module-level clusters (tracking, scene graph) into the handle.
