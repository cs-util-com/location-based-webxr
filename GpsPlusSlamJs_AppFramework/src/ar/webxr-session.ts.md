...
- `activeSession` — the per-session `ArSessionHandle` (**Stage 0** of the [staged ArSession refactor](../../../../gps-plus-slam/GpsPlusSlamJs_Docs/docs/2026-07-18-2045-webxr-session-arsession-handle-refactor-plan.md)). Owns this session's low-coupling clusters: the resolved crash-isolation options, the session-end pair (`onSessionEnd` callback + `endRequestedByApp` discriminator), and the frame diagnostics (`lastFrameTime`, `latestCameraTexture`/width/height, camera-access log latches). Always non-null in Stage 0: `initAR()` creates a fresh parameterized handle and `resetWebXRState()` REPLACES it with a default one — one line resets every field on it, so new handle state can never be forgotten in the teardown. Later stages migrate the remaining module-level clusters (capture, tracking, scene graph) into the handle.
