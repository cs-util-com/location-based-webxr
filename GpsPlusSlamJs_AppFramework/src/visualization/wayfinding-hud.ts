 ...
 *   no imperative setWaypoints/addWaypoint/removeWaypoint API.
...
export interface WayfindingHudOptions {
  ...
  /**
   * Polled once per frame; returns the current target world positions.
   * Per-target state is INDEX-based: when the list shrinks, trailing
   * states are disposed; when it grows, fresh 'hidden' states are created.
   * An identity change at a constant index is not detected (the hysteresis
   * state machine simply continues from the previous state).
   */
...
function validateHudRefs(options: WayfindingHudOptions): void {
  ...
  if (typeof options.getTargets !== 'function') {
    throw new TypeError(
      'createWayfindingHud: getTargets must be a function returning THREE.Vector3[]'
    );
  }
}
...
export function createWayfindingHud(
  ...
  function syncTargetCount(targetCount: number): void {
    for (let i = states.length; i < targetCount; i += 1) {
      states.push(makeState());
    }
    // Shrink: dispose trailing states (getter-API replacement for the
    // prototype's removeWaypoint splice — see the sidecar).
    while (states.length > targetCount) {
      disposeState(states.pop() as TargetState);
    }
  }
  ...
  function showArrow(state: TargetState, placement: ArrowPlacement): void {
    ...
    state.arrow.position.copy(placement.arrowPosition);
    if ((state.arrow as THREE.Sprite).isSprite) {
      (state.arrow as THREE.Sprite).material.rotation =
        placement.arrowRotationZ;
    } else {
      state.arrow.rotation.set(0, 0, placement.arrowRotationZ);
    }
  }
  ...
  function readTargets(): THREE.Vector3[] {
