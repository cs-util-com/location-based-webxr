...
- **Best-effort visual layers:** the frame-tile visualizer (F3.5) and the occupancy-grid cubes (2026-06-11 depth occupancy-grid port plan, Iter 5) are each wired inside their own `try/catch` — a failure (e.g. zip without `frames/`, WebGL issue) logs a warning and replay continues without that layer. Both layers are built by the stacks shared with the live scene (`visualization/occupancy-stack.ts` with `showCubes: true` and `logContext: 'during replay'`, `visualization/frame-tile-stack.ts` with no tile cap) — since 2026-09-04 one function each, so the carve floor, noise floor, camera-move epsilon and refresh throttle cannot drift from live. Re-dispatched `recordDepthSample` actions rebuild the voxel grid through that stack; recordings made before intrinsics capture have no `projectionMatrix`, so their grid simply stays empty. The cube visualizer is parented under `replaySceneState.arWorldGroup` (NOT the scene root) because the grid's cells are raw-WebXR coordinates that must ride the alignment matrix like the recorded camera path (port plan Iter 7). Both layers are torn down in `dispose()`.
