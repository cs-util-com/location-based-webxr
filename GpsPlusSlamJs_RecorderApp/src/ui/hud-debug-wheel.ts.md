...
- **Selecting a preset dispatches TWO actions: the setting, and a `diagnostics/note` naming it.** The override payload alone does not say which preset was tapped, and a preset removed later cannot be reverse-matched from its payload at all - seven were removed on 2026-09-12, and replaying a recording made with one of them now shows `shipped` in the wheel. The note is dispatched on every change rather than captured once at session start, because switching mid-walk is the wheel's entire purpose and a start-of-session snapshot would confidently assert a preset that was active for the first thirty seconds. The note has no reducer and changes nothing.
