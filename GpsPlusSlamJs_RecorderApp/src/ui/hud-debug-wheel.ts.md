...
- `hud-debug-wheel.test.ts` (jsdom) — per control: the preset is one action, the slider the seven (prior on at the weight; three-setting silence at 0), the gate / pair selection / prerequisite / penalty their own setters, all eleven together; seeding from a decided store (and never over a touched control, never before decided); mount/toggle/dispose; nothing dispatched while untouched; a preset-only tap leaves the compass config alone; a change before the first fix flushed once decided and not inside the deciding dispatch; re-apply of every touched control on a store swap, once per store; suspend/resume around a replay store; slider on release; the disabled penalty box; the readout live while open; dash-not-NaN and yaw wrap.
