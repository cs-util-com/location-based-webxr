...
- Every step is `|bearingDeltaDeg(yaw, lastYaw)|` (the framework's `utils/bearing-degrees.ts`, so 359° → 1° is a 2° step, not 358°), hence in [0, 180]; the median is the framework's `interpolatingMedian` (`utils/median.ts`): the middle value, the mean of the two middle values for an even count. Both helpers were private copies here until 2026-09-04.
...
- Pure and allocation-light: the sorted copy is made only in `summary()` (inside `interpolatingMedian`), which runs on readout refresh, not per store update.
...
- `yaw-churn.test.ts` — one sample per fix (repeated fix counts ignored), wrap-around steps, non-finite yaw skipped without breaking the chain, the window keeps the last 30 steps, `null` before two samples, the window guard. The signed-delta cases moved to the framework's `bearing-degrees.test.ts` with the helper.
