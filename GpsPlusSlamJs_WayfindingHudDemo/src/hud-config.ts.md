...
- **Public API:**
  - `HudDemoConfig` — `{ distanceMin, distanceMax, indicatorScale }`.
  - `AR_HUD_CONFIG` (1.5 / 3.0 / 1.0, real-world walking) and `SIM_HUD_CONFIG` (8 / 12 / 1.0, simulator scale — waypoints sit 10–25 m out).
  - `sanitizeHudDemoConfig(raw, fallback)` — non-finite fields → fallback; `distanceMin` clamped ≥ 0; `distanceMax` clamped ≥ `distanceMin`; scale clamped to [0.1, 5].
