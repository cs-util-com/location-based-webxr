...
- **Wiring:**
  ...
  - **AR:** "Start AR" → sliders initialise to `AR_HUD_CONFIG` → `startArMode`; `onStarted` reveals the HUD panel + tap hint, `onError` re-enables the button with the reason, `onEnded` (system back gesture) returns to the mode screen.
